# TunnelVision — Architecture Deep Dive
...
TunnelVision is a high-performance network tunneling tool written in Go. It allows arbitrary TCP, UDP, SOCKS5, HTTP, Unix Socket, and Stdio traffic to be tunneled over WebSocket or HTTP/2 connections, making it effective at bypassing restrictive firewalls and proxies that only permit standard HTTPS traffic.

---

## High-Level Architecture

TunnelVision operates as two cooperating processes:
...
```
tunnelvision/
├── cmd/tunnelvision/          # CLI entrypoint (main.go, main_test.go)
├── pkg/
...
TunnelVision supports three distinct transport modes:
...
## Data Flow Example: SSH over TunnelVision
