# tunnelvision Protocol Technical Specification

## Overview
`tunnelvision` is a tunneling protocol designed to encapsulate arbitrary layered-4 traffic (TCP, UDP, SOCKS5, HTTP Proxy, Unix Sockets) within a WebSocket or HTTP/2 transport stream. Its primary purpose is to bypass restrictive firewalls and Deep Packet Inspection (DPI) systems by making the tunneled traffic appear as standard web traffic.
...
Instead of a separate control plane, `tunnelvision` encodes the tunnel instructions within the `Sec-WebSocket-Protocol` header.
...
To improve performance and bypass certain DPI heuristics, `tunnelvision` intentionally deviates from RFC 6455 in several ways:

### 1. Relaxed Client Masking
RFC 6455 Section 5.1 requires that all frames sent from a client to a server must be masked.
- **Deviation:** The `tunnelvision` server implementation is permissive and **accepts unmasked frames** from the client.
...
4.  **Piping:** Both sides enter a loop using the `pkg/tunnel` (Go) or `tunnelvision::tunnel::transport::io` (Legacy) logic.
...
```bash
tunnelvision client -L tcp://1212:google.com:443 --jwt-secret mysecret ws://server:8080
```

### Server Configuration (Go)
```bash
tunnelvision server ws://0.0.0.0:8080 --jwt-secret mysecret
```
