...
func init() {
	var err error
	rustBinary, err = exec.LookPath("tunnelvision")
	if err != nil {
		fmt.Printf("Rust binary 'tunnelvision' not found in PATH: %v\n", err)
...
func testReverseTCP(t *testing.T, listenPort int, echoServerPort int) {
	...
	// But in tunnelvision terms, -R tcp://listenPort:localhost:echoServerPort
...
func TestInteroperability(t *testing.T) {
	if rustBinary == "" { // Check if exec.LookPath found the binary
		t.Skipf("Rust binary 'tunnelvision' not found in PATH. Skipping Rust interoperability tests.")
	}
