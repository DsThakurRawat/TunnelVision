...
func TestReverseSocks5_DynamicTargetE2E(t *testing.T) {
	...
	// 2. Start TunnelVision server
	srvLn, err := net.Listen("tcp", "127.0.0.1:0")
	if err != nil {
		t.Fatalf("Failed to listen TunnelVision server: %v", err)
	}
	...
	// 4. Start TunnelVision client requesting reverse SOCKS5 tunnel
...
func TestReverseHttpProxy_DynamicTargetE2E(t *testing.T) {
	...
	// 4. Start TunnelVision client requesting reverse HTTP proxy tunnel
	wstClient := client.NewClient(client.Config{
		ServerURL:                              srvAddr,
		ReverseTunnelConnectionRetryMaxBackoff: 1 * time.Second,
	})
