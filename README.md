...
A feature-complete Go implementation of [tunnelvision](https://github.com/erebe/tunnelvision), designed for high performance, ease of use, and library integration.
...
tunnelvision client -R socks5://0.0.0.0:1080 wss://my-server.com

# Reverse HTTP CONNECT proxy (Go-to-Go only)
tunnelvision client -R http://0.0.0.0:3128 wss://my-server.com

# Preserve the client IP to the backend via a PROXY v2 header
tunnelvision client -L "tcp://8080:backend.internal:80?proxy_protocol" wss://my-server.com

# UDP tunnel with a 15s idle-session timeout
tunnelvision client -L "udp://5353:1.1.1.1:53?timeout_sec=15" wss://my-server.com

# Transparent proxy (Linux, needs CAP_NET_ADMIN + an iptables rule — see below)
tunnelvision client -L tproxy+tcp://0.0.0.0:1234 wss://my-server.com
...
kill -HUP "$(pidof tunnelvision)"   # also reloads automatically within a few seconds
```
...
`tunnelvision` aims for 100% parity with the [Rust version](https://github.com/erebe/tunnelvision).
...
| Metric | tunnelvision (Rust) | tunnelvision |
| :--- | :---: | :---: |
