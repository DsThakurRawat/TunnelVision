...
5. **CLI flags** are registered in `cmd/tunnelvision/main.go` (client flags start
...
- **Source address** to advertise: the remote address of the tunnel connection
  (the tunnelvision *client*), obtained from the underlying websocket/h2 request.
...
(`X-TunnelVision-Src: ip:port`) in `connectToTunnelVision`/`connectToHttp2`/
...
### Acceptance
- `tunnelvision server --tls` (no cert/key) serves working TLS; explicit cert/key
...
2. **CLI usage strings** — implement real `Usage:` text for every new flag added
   ...
   `cmd/tunnelvision/main.go`, and make sure `tunnelvision client --help` /
   `tunnelvision server --help` render them. Add a one-line example to each flag's
