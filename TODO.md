...
[x] Add support for running tunnelvision via systemd
...
[x] Add support (scripts) for starting tunnelvision client on Windows via task scheduler
...
[x] Caddy integration (server):
   - consider example implementation of caddy app plugin that can be built into caddy server to allow serving tunnelvision tunnels.
