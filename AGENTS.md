...
This document outlines the established rules, conventions, and processes for the `tunnelvision` project. It serves as a guide for both human developers and AI agents to ensure consistency, quality, and efficient development.
...
-   `cmd/tunnelvision/main.go`: Contains the main application entry point.
...
-   `test-interop`: Runs end-to-end interoperability tests between `tunnelvision` and compatible implementations.
...
-   `clean`: Removes build artifacts, including the `tunnelvision` binary and any other temporary files.
...
-   **Purpose:** To validate compatibility with the original compatible implementations.
